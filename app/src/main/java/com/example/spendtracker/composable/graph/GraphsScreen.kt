...
fun <T : Graph> GraphsScreen(
    ...
    val sortedInvestments = remember(items, selectedSortOption) {
        when (selectedSortOption) {
            SortOption.DATE_ASC -> items.sortedBy { it.date }
            SortOption.DATE_DESC -> items.sortedByDescending { it.date }
            SortOption.CATEGORY -> items.sortedBy { it.category }
            SortOption.AMOUNT_ASC -> items.sortedBy { it.amount }
            SortOption.AMOUNT_DESC -> items.sortedByDescending { it.amount }
        }
    }
